...
int main(){
	...
	printf("Bem vindo ao Sistema Estatistico de Uso do Salao de Estudos");
	...
	for (i = 0; i < tamn; i++){
		...
		if(achou){
			...
			sis[0][achouvalor] = 0; // zerando o nusp, para não considerar nas comparacoes futuras
			k++;
    ...
    for (i = 0; i < k; i++)
   	...
   	// Ordenando a tabela pelo tempo de permanencia. Coloca em aux os indices de correção das variáveis ordenadas
	...
	// Ordenando a tabela pelo horário de entrada. . Coloca em aux os indices de correção das variáveis ordenadas
...
void distribuicao_por_tempo_diario(int dados[3][MAX], int tamn, int unicos[MAX], int tamun){
	...
	printf("\n    PERIODO    |     USUARIOS");
	for (i = 0; i < 48; i++){
		printf("\n %02d:%02d - %02d:%02d | ", ((i * 30) / 60), ((i * 30) % 60) + 1, (((i+1) * 30) / 60) % 24, ((i+1) * 30) % 60);
...
void ordena(int dados[3][MAX], int tamn, int col, int ordena[], int ascdesc){
    ...
    for(i = 0; i < tamn; i++)
        for(j = 0; j < tamn; j++){
        	...
        	// Para garantir que nao haverao problemas com multiplas entradas de estudantes, ha a condicao ordena[j] == -1 e um break
